# trading/management/commands/sync_ibkr_positions.py
...
class Command(BaseCommand):
    help = "Sync positions from IBKR for all LIVE and PAPER_LINKED BrokerAccounts."
    ...
    def handle(self, *args, **options):
        ...
        total_positions = 0
        self.stdout.write(
            f"Syncing positions for {qs.count()} broker account(s)..."
        )

        for ba in qs:
            self.stdout.write(f"  → {ba.account_code} ({ba.kind}) ... ", ending="")
            try:
                created = sync_positions_for_broker_account(ba)
                total_positions += created
                self.stdout.write(self.style.SUCCESS(f"OK ({created} positions)"))
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Completed positions sync. Created {total_positions} Position snapshot(s)."
            )
        )
